...
像上面这样写乍一看好像没问题，对比一下两种写法，其实就是回溯`visit`的时机不同，第一个是下一层`dfs` 不管成功与否都会回溯状态，而第二种是下一层`dfs` 成功后才会回溯状态，失败后就不会回溯状态了，但是在这题中，很明显不管失败与否都需要回溯状态，因为你这里**下一层**的`dfs` 成功意味着**当前层**的`dfs`的这次尝试失败了，但是我还要继续尝试，你下一层成功之后如果不回溯`visit` ，当前层就用不了被下层占用的数字，就无法枚举所有情况
